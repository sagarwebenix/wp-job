...
class WP_Job_Board_Recaptcha {
	...
	public static function validate_fields( $return ) {
		if ( self::is_recaptcha_enabled() ) {
			...
			if ( !$is_recaptcha_valid ) {
				return new WP_Error( 'validation-error', esc_html__( 'reCAPTCHA is a required field', 'wp-job-board' ) );
			}
		}
		return $return;
	}
	...
	public static function is_recaptcha_enabled() {
		$site_key = wp_job_board_get_option( 'recaptcha_site_key' );
		$secret_key = wp_job_board_get_option( 'recaptcha_secret_key' );
	...
	public static function is_recaptcha_valid( $recaptcha_response ) {
		$response = wp_remote_get(
			add_query_arg(
				array(
					'secret'   => wp_job_board_get_option( 'recaptcha_secret_key' ),
