...
class WP_Job_Board_User {
	...
	public static function process_register() {
        ...
        self::registration_validation( $_POST['username'], $_POST['email'], $_POST['password'], $_POST['phone'], $_POST['confirmpassword'] );
        if ( 1 > count( $reg_errors->get_error_messages() ) ) {
	        ...
	        if ( ! empty( $_POST['cif_field'] ) && intval( $_POST['cif_field'] )) {
				update_user_meta( $user_id, 'cif_field', intval( $_POST['cif_field'] ) );
			}
	...
	public static function registration_validation( $username, $email, $password, $phone, $confirmpassword, $captcha = true ) {
		global $reg_errors;
	...
	public static function registration_save($user_id) {
        ...
        if ($user_role == 'wp_job_board_employer') {
	        ...
	        if ( isset($_POST['cif_field']) ) {
	            update_post_meta($employer_id, WP_JOB_BOARD_EMPLOYER_PREFIX . 'cif_number', $_POST['cif_field']);
	        }
