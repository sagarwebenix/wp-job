...
class WP_Job_Board_Widget_Employer_Filter extends WP_Widget {
	/**
	 * Initialize widget
	 *
	 * @access public
	...
	function __construct() {
		parent::__construct(
			...
			__( 'Employer Filter', 'wp-job-board' ),
			array(
				'description' => __( 'Filter for filtering employers.', 'wp-job-board' ),
			)
		);
	}
	...
	function widget( $args, $instance ) {
		include WP_Job_Board_Template_Loader::locate( 'widgets/employer-filter' );
	}
	...
	function form( $instance ) {
		...
		<ul class="wp-job-board-filter-fields wp-job-board-filter-employer-fields">
			<?php

			$all_fields = $fields = WP_Job_Board_Employer_Filter::get_fields();
			if ( ! empty( $sort ) ) {
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				$('.widget .wp-job-board-filter-employer-fields').each(function() {
					var el = $(this);
...
register_widget('WP_Job_Board_Widget_Employer_Filter');