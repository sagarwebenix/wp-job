...
class WP_Job_Board_Taxonomy_Candidate_Category{
	...
	public static function definition($args) {
		...
		$rewrite_slug = get_option('wp_job_board_candidate_category_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'candidate-category', 'Candidate category slug - resave permalinks after changing this', 'wp-job-board' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'candidate_category', 'candidate', array(
			'labels'            => apply_filters( 'wp_job_board_taxomony_candidate_category_labels', $labels ),
			'hierarchical'      => true,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Job_Board_Taxonomy_Candidate_Category::init();