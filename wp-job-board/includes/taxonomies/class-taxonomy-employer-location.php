...
class WP_Job_Board_Taxonomy_Employer_Location{
	...
	public static function definition() {
		...
		$rewrite_slug = get_option('wp_job_board_employer_location_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'employer-location', 'Employer location slug - resave permalinks after changing this', 'wp-job-board' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'employer_location', 'employer', array(
			'labels'            => apply_filters( 'wp_job_board_taxomony_employer_location_labels', $labels ),
			'hierarchical'      => true,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Job_Board_Taxonomy_Employer_Location::init();