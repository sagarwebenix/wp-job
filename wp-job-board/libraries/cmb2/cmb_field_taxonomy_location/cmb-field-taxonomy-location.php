...
/**
 * Class WP_Job_Board_CMB2_Field_Taxonomy_Location
 */
class WP_Job_Board_CMB2_Field_Taxonomy_Location {
	...
	public function render_taxonomy_location( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		$nb_fields = apply_filters('wp_job_board_cmb2_field_taxonomy_location_number', 4);
		$parent = 0;
		...
		for ($i=1; $i <= $nb_fields; $i++) {
			...
			$field_name = apply_filters('wp_job_board_cmb2_field_taxonomy_location_field_name_'.$i, 'Country');
	...
	public function setup_admin_scripts() {
		...
		wp_enqueue_script( 'wpjb-taxonomy-location-script', $asset_path . '/js/script.js', array( 'cmb2-scripts', 'select2', 'jquery-ui-sortable' ), self::VERSION );
		wp_localize_script( 'wpjb-taxonomy-location-script', 'location_opts', array(
			'ajaxurl' => admin_url( 'admin-ajax.php' ),
			'ajax_nonce' => wp_create_nonce( 'wpjb-ajax-nonce' )
		));
		wp_enqueue_style( 'wpjb-taxonomy-location-style', $asset_path . '/css/style.css', array( 'select2' ), self::VERSION );
	}
...
$wpjb_cmb2_field_select2 = new WP_Job_Board_CMB2_Field_Taxonomy_Location();
