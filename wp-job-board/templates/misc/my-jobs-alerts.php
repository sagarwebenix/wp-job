...
<div class="box-employer widget">
	<h3 class="widget-title"><?php echo esc_html__('Job Alerts','wp-job-board') ?></h3>
			...
			<table class="job-table">
				...
				<?php foreach ($alerts->posts as $alert_id) {
					...
					<tr <?php post_class('job-alert-wrapper'); ?>>
						...
						<td>
							<div class="job-found">
					            <?php echo sprintf(esc_html__('Jobs found %d', 'wp-job-board'), intval($count_jobs) ); ?>
					        </div>
						</td>
						<td>
							<div class="job-metas">
	...
	<?php } else { ?>
		<div class="not-found"><?php esc_html_e('No job alert found.', 'wp-job-board'); ?></div>
	<?php } ?>
</div>